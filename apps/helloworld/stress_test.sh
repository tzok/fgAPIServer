...
do
  ...
  TMP=$(mktemp)
  CMD="curl -H \"Content-Type: application/json\" -H \"Authorization: $SESSION_TOKEN\" -X POST -d '{\"application\":\"1\",\"description\":\"helloworld@csgfsdk test run\" }' http://$FGHOST:$FGPORT/$FGAPIV/tasks$FILTER 2>/dev/null >$TMP"
  echo "Executing: $CMD"
  eval $CMD
  ...
  if [ "$status" == "WAITING" ]; then
    CMD="curl -H \"Authorization: $SESSION_TOKEN\" -X POST http://$FGHOST:$FGPORT/$FGAPIV/tasks/$id/input$FILTER"
	echo "Executing: $CMD"
	eval $CMD
  fi
  rm -f $TMP
